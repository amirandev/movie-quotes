@extends('admin.partials.layout', ['pagetitle' => __('main.movies')])
@section('content')
    <div class="breadcrumb-container overflow-auto">
        <div class="btn-group btn-breadcrumb">
            <a href="{{ route('dashboard') }}" class="btn btn-default"><i class="las la-home"></i></a>
            <a href="{{ route('admin.movies') }}" class="btn btn-default">{{ __('main.directors') }}</a>
        </div>
    </div>

    <style>
        .smalltextAuthor, .smalltextAuthor *{
    ...
    <section class="p-normal bg-white mt-3 filterAndResults">
        <div class="mt-0 parent-gtu">
            <h1>{{ __('main.manage_movies') }}</h1>
            <div class="text-muted">
                {{ __('main.manage_movies_mutedText') }}
            </div>
            <div class="pt-3">
            ...
            <div class="ms-3 me-3">
                <div class="input-group">
                    ...
                    <select class="form-select form-select-lg minimisedSelect" name="sort" id="sort">
                        ...
                        <option value="3">{{ __('main.title') }} ({{ __('main.ascending') }})</option>
                        <option value="4">{{ __('main.title') }} ({{ __('main.descending') }})</option>
                    </select>
                </div>
            </div>
            <div class="ms-3 me-3">
    ...
    <section class="filterAndResults">
        <div class="movies mt-3" id="load_content" data-source="{{ route('admin.movies.json') }}">
            <!--results here-->
        </div>
