<script>
    ...
    $(document).on('click', '.askdelete', function(e){
        ...
        if(confirm('{{ __('main.questionDeleteQuote') }}')){
            $.ajax({
                type: 'post',
                url: '{{ route('admin.soft_delete_quote') }}',
                data: {
                    _token: '{{ csrf_token() }}',
                    id:id
                },
    ...
    $(document).on('submit', '#editform', function(e){
        ...
        console.log({
                _token: '{{ csrf_token() }}',
                text_en: $('#text_en2').val().trim(),
                text_ka: $('#text_ka2').val().trim(),
                thumbnail: $('#thumbnail2').val().trim(),
                movie: $('#movie2').val().trim(),
        });
